...
// Pods 2.0 will include Pods UI by default, you don't need this plugin anymore
if (function_exists('pods_ui_manage') || (defined('PODS_VERSION') && PODS_VERSION > 198)) {
...
<?php
    ...
    function pods_ui_validate_plugin () {
        if (!function_exists('pod_query')) {
            add_thickbox();
            add_action('admin_notices', 'pods_ui_validate_plugin_notice');
            return false;
        }
        return true;
    }
    function pods_ui_validate_plugin_notice () {
        $this_plugin = 'Pods UI';
        if (!function_exists('pod_query')) {
...
?>
    <div class="error fade">
        <p>The <?php echo $plugin_name; ?> plugin is required for the <?php echo $this_plugin; ?> plugin to function properly. <a href="<?php echo admin_url('plugin-install.php?tab=plugin-information&plugin=' . $plugin_slug . '&TB_iframe=true&width=640&height=517'); ?>" class="thickbox onclick">Install now</a> and Activate.</p>
    </div>
<?php
        }
    }
...
{
    if (!pods_ui_validate_plugin())
        pods_ui_validate_plugin_notice();
